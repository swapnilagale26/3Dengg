...
class user_edit_form extends \moodleform {
    ...
    public function definition() {
        ...
        //Manju: Extra fields for users. starts.
        //1. Branch.
        $mform->addElement('text', 'firstnamephonetic', 'Branch');
        $mform->setType('firstnamephonetic', PARAM_RAW);

        $mform->addElement('text', 'address', 'Domain or Expertise');
        $mform->setType('address', PARAM_RAW);
        //$mform->addRule('firstnamephonetic', get_string('required'), 'required', null, 'client');

        //2. Contact Number.
        $mform->addElement('text', 'phone1', 'Contact Number');
        $mform->setType('phone1', PARAM_RAW);
        $mform->addRule('phone1', get_string('required'), 'required', null, 'client');

        //3. Birth date.
        $mform->addElement('date_time_selector', 'skype', 'Birth Date');
        $mform->setType('skype', PARAM_RAW);
        //$mform->addRule('skype', get_string('required'), 'required', null, 'client');

        //4. Semister.
        $semarray = array('-',1,2,3,4,5,6,7,8);
        $mform->addElement('select', 'icq','Semister', $semarray);
        
        //5. College Name
        $mform->addElement('text', 'lastnamephonetic', 'College Name');
        $mform->setType('lastnamephonetic', PARAM_TEXT);

        //6. College Enrolment Number
        $mform->addElement('text', 'aim', 'College Enrolment Number');
        $mform->setType('aim', PARAM_RAW);

        //7. Adhar Number.
        $mform->addElement('text', 'yahoo', 'Adhar Number');
        $mform->setType('yahoo', PARAM_RAW);

        //Manju: Extra fields for users. ends.
        ...
        $mform->addElement('hidden', 'departmenttext', 'Institute');
