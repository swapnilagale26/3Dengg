...
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        ...
        foreach ($userstoassign as $adduser) {
            //check for user role.
            $sql = "SELECT * FROM {role_assignments} WHERE userid = ".$adduser->id." AND roleid = 3";
		    $trainer = $DB->get_records_sql($sql);
		    if(!empty($trainer)){
		        cohort_add_member($cohort->id, $adduser->id);
		    }else{
		   //get the cohort size here.
            $size = $DB->get_field('cohort','size',array('id'=>$cohort->id));
            
            //get the current size of the cohort.
            $currentusers = $DB->get_records('cohort_members',array('cohortid'=>$cohort->id));
            $currentcount = count($currentusers);
            if($currentcount >= $size){
                $html="";
                $html.='<script>alert("Maximum user limit is reached");</script>';
                echo $html;
                continue;
            }else{
                 cohort_add_member($cohort->id, $adduser->id);
