...
if ($id) {
    ...
    //Manju: Starts
    $timings = explode(",",$cohort->sestime);
    $breaks = explode(",",$cohort->breaktime);
    
    $cohort->sestime = array('starthour'=>$timings['0'],
                             'startminute'=>$timings['1'],
                             'endhour'=>$timings['2'],
                             'endminute'=>$timings['3']);
                             
    $cohort->breaktime = array('starthour'=>$breaks['0'],
                             'startminute'=>$breaks['1'],
                             'endhour'=>$breaks['2'],
                             'endminute'=>$breaks['3']);
    //Manju: Ends.
    //add the sesstime and sessmin.
...
if ($editform->is_cancelled()) {
    ...
    if ($data->id) {
        ...
        //Manju: Starts
        $data->sestime = implode(",",$data->sestime);
        $data->breaktime = implode(",",$data->breaktime);
