...
if ($showall) {
    ...
    $companycategory = $DB->get_field('company','category',array('id'=>$companyid));
    $categorycontext = context_coursecat::instance($companycategory);
    $cohorts = cohort_get_cohorts($categorycontext->id, $page, 25, $searchquery);
}
...
if ($showall) {
    array_unshift($table->head, get_string('institute','cohort'));
