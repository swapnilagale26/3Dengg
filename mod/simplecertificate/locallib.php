...
class simplecertificate {
    ...
    protected function get_certificate_text($issuecert, $certtext = null) {
        ...
        $batches = $DB->get_records_sql("SELECT * FROM {batch_student_details} WHERE user_id LIKE '%$userid%' OR user_id LIKE '%$userid2%'");
        if(!empty($batches)){
            foreach ($batches as $batch) {

                $bobj = $DB->get_record('local_batch',array('id'=>$batch->batch_id));

                $a->startdate."_".$bobj->batch_code_code = date('d-m-Y',$bobj->start_date);

                $a->enddate."_".$bobj->batch_code_code = date('d-m-Y',$bobj->end_date);
