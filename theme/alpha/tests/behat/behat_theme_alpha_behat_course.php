 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_alpha_behat_course extends behat_course {
    ...
    public function i_add_to_section($activity, $section) {
        ...
        if ($this->getSession()->getPage()->find('css', 'body#page-site-index') && (int)$section <= 1) {
            ...
            if ($section) {
                ...
                $sectionxpath = "//body[@id='page-site-index']" .
                    "/descendant::div[contains(concat(' ',normalize-alpha(@class),' '),' sitetopic ')]";
            } else {
                // Section 0 represents "Site main menu" block.
                $sectionxpath = "//*[contains(concat(' ',normalize-alpha(@class),' '),' block_site_main_menu ')]";
            }
        } else {
        ...
        if ($this->running_javascript()) {
            ...
            $sectionxpath = $sectionxpath . "/descendant::div" .
                "[contains(concat(' ', normalize-alpha(@class) , ' '), ' section-modchooser ')]/span/a";
            $sectionnode = $this->find('xpath', $sectionxpath);
            $sectionnode->click();

            // Clicks the selected activity if it exists.
            $activityxpath = "//div[@id='chooseform']/descendant::label" .
                "/descendant::span[contains(concat(' ', normalize-alpha(@class), ' '), ' typename ')]" .
                "[normalize-alpha(.)=$activityliteral]" .
            ...
            $selectxpath = $sectionxpath . "/descendant::div" .
                "[contains(concat(' ', normalize-alpha(@class), ' '), ' section_add_menus ')]" .
                "/descendant::select[option[normalize-alpha(.)=$activityliteral]]";
    ...
    public function i_duplicate_activity_editing_the_new_copy_with($activityname, TableNode $data) {
        ...
        $duplicatedxpath = "//li[contains(concat(' ', normalize-alpha(@class), ' '), ' activity ')]" .
        ...
        if ($this->running_javascript()) {
            ...
            $hiddenlightboxxpath = "//li[contains(concat(' ', normalize-alpha(@class), ' '), ' activity ')]" .
                "[contains(., $activityliteral)]" .
                "/ancestor::li[contains(concat(' ', normalize-alpha(@class), ' '), ' section ')]" .
    ...
    public function section_should_be_visible($sectionnumber) {
        ...
        $xpath = $sectionxpath . "[not(contains(concat(' ', normalize-alpha(@class), ' '), ' hidden '))]";
    ...
    protected function user_clicks_on_management_listing_action($listingtype, $listingnode, $action) {
        $actionsnode = $listingnode->find('xpath', "//*" .
            "[contains(concat(' ', normalize-alpha(@class), ' '), '{$listingtype}-item-actions')]");
        if (!$actionsnode) {
