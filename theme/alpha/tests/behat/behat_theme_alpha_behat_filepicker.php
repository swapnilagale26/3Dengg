...
require_once(__DIR__ . '/behat_theme_alpha_behat_files.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_alpha_behat_filepicker extends behat_theme_alpha_behat_files {
    /**
    ...
    public function i_open_folder_from_filemanager($foldername, $filemanagerelement) {
        ...
        try {
            ...
            // In the current folder workalpha.
            $folder = $this->find(
                'xpath',
                "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-folder ')]" .
                    "/descendant::div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-filename ')]" .
                    "[normalize-alpha(.)=$folderliteral]",
                $exception,
                $fieldnode
            );
            ...
            $folder = $this->find(
                'xpath',
                "//a[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-path-folder-name ')]" .
                    "[normalize-alpha(.)=$folderliteral]",
                $exception,
                $fieldnode
            );
    ...
    public function i_should_see_elements_in_filemanager($elementscount, $filemanagerelement) {
        ...
        $xpath = "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' filemanager ')]" .
            "[not(contains(concat(' ', normalize-alpha(@class), ' '), ' fm-updating '))]" .
            "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-content ')]" .
            "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-file ')]";
