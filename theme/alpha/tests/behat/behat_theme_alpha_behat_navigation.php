 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_alpha_behat_navigation extends behat_navigation {
    ...
    protected function get_top_navigation_node($nodetext) {
        ...
        $xpath = "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' card-text ')]" .
            "/ul[contains(concat(' ', normalize-alpha(@class), ' '), ' block_tree ')]" .
            "/li[contains(concat(' ', normalize-alpha(@class), ' '), ' contains_branch ')]" .
            "/ul/li[contains(concat(' ', normalize-alpha(@class), ' '), ' contains_branch ')]" .
            "[p[contains(concat(' ', normalize-alpha(@class), ' '), ' branch ')]" .
            "/*[contains(normalize-alpha(.), " . $nodetextliteral .")]]" .
            "|" .
            "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' card-text ')]/div" .
            "/ul[contains(concat(' ', normalize-alpha(@class), ' '), ' block_tree ')]" .
            "/li[p[contains(concat(' ', normalize-alpha(@class), ' '), ' branch ')]" .
            "/*[contains(normalize-alpha(.), " . $nodetextliteral .")]]";
    ...
    protected function select_on_administration_page($nodelist) {
        ...
        if ($parentnodes) {
            ...
            $tabxpath = '//ul[@role=\'tablist\']/li/a[contains(normalize-alpha(.), ' . $tabname . ')]';
        ...
        $xpath .= '//a[contains(normalize-alpha(.), ' . $linkname . ')]';
    ...
    protected function select_from_administration_menu($nodelist) {
        ...
        if (!$isheader || count($nodelist) == 1) {
            ...
            $linkname = behat_context_helper::escape($lastnode);
            $link = $this->getSession()->getPage()->find('xpath', $menuxpath . '//a[contains(normalize-alpha(.), ' . $linkname . ')]');
            if ($link) {
                $link->click();
                $this->wait_for_pending_js();
                return;
