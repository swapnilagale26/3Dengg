 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_alpha_behat_repository_upload extends behat_repository_upload {
    ...
    protected function get_filepicker_node($filepickerelement) {
        ...
        if (empty($filepickerelement)) {
            ...
            $filepickercontainer = $this->find(
                'xpath',
                "//input[./@id = //label[normalize-alpha(.)=$filepickerelement]/@for]" .
                    "//ancestor::div[contains(concat(' ', normalize-alpha(@class), ' '), ' felement ')]",
                $exception
            );
        }
    ...
    protected function upload_file_to_filemanager($filepath, $filemanagerelement, TableNode $data, $overwriteaction = false) {
        ...
        $this->find(
            'xpath',
            "//div[contains(concat(' ', normalize-alpha(@class), ' '), ' container ')]" .
                "[contains(concat(' ', normalize-alpha(@class), ' '), ' repository_upload ')]" .
                "/descendant::div[contains(concat(' ', normalize-alpha(@class), ' '), ' file-picker ')]" .
                "/descendant::div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-content ')]" .
                "/descendant::div[contains(concat(' ', normalize-alpha(@class), ' '), ' fp-upload-form ')]" .
                "/descendant::form",
            $noformexception
        );
