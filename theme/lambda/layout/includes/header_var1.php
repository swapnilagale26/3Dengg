...
<div class="container-fluid">    
	<div class="row-fluid">
		...
		<div class="span6">
			<div class="logo-header">
				<a class="logo" href="<?php echo $CFG->wwwroot; ?>" title="<?php print_string('home'); ?>">
			        ...
			        $fs = get_file_storage();
			        $files = $fs->get_area_files(1, 'theme_iomad', 'companylogo', $companyid );
			        if ($files) {
			            foreach ($files as $file) {
			                $filename = $file->get_filename();
			                $filepath = ((int) $maxwidth . 'x' . (int) $maxheight) . '/';
			                if ($filename != '.') {
			                    $setting ="http://3dengglms.com/pluginfile.php/1/theme_iomad/companylogo/".$filepath.$companyid."/".$filename;
			                }
			            }
			        }else{
			            $setting = $PAGE->theme->setting_file_url('logo', 'logo');
			        }
			        echo html_writer::empty_tag('img', array('src'=>$setting, 'class'=>'img-responsive', 'alt'=>'logo'));
		...
		<div class="span6">
			<div class="title-text">
				<h5 id="title">Learning Management System</h5>
			</div>
		</div>    	
